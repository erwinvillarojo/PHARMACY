    ...
    def post(self, request):
        if request.method == 'POST':    
            ...
            if 'btnUpdate' in request.POST:
                ...
                update_m = Medicine.objects.filter(id = mid).update(no_of_items = i, order = order, total = total) 
    ...
    def post(self, request):
        ...
        if form.is_valid():
             ...
             form = Medicine(date_registered = dregistered, sku = stock_unit, category = category, generic_name = generic, common_brand= common, 
                             ...
                             how_to_use = use, img1 = image1, img2 = image2, img3 = image3, order =order, total =total)
